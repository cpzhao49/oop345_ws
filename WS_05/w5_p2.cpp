...
/*Hi professor, I'm the student writing this workshop. When implementing the TODO functions in this file I took the w4p2.cpp from last workshop as a reference
to write file reading related functions. I'm not sure if this is allowed so I left this message.*/
#include <iostream>
#include <iomanip>
#include <fstream>
...
int main(int argc, char** argv)
{
	...
	auto newPrice = [usdToCadRate, gbpToCadRate](Book& book) {
		if (book.country() == "US") {
	...
	if (argc > 2) {
		...
		do
		{
			...
			if (file && movieLine[0] != '#')
			{
				movies[l_count] = Movie(movieLine);
				l_count++;
			}
		} while (file);
	}
	...
	try {
		...
		std::cout << "** EXCEPTION: SOME ERR" << std::endl;
	}
